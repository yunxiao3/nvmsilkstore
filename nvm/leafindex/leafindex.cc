...
class LeafIndexIterator: public Iterator {
 ...
 private:
  ...
  LeafIndex::Index::unsafe_iterator iter_;
...
IndexIterator LeafIndex::NewIndexIterator() {
...
Status LeafIndex::AddBatch(const NvmWriteBatch* batch){
...
int bytesToInt_64(const char* bytes){
   /*  size_t addr = (size_t)bytes[7] & 0xFF;
    addr |= (((size_t)bytes[6] << 8) & 0xFF00);
    addr |= (((size_t)bytes[5] << 16) & 0xFF0000);
    addr |= (((size_t)bytes[4] << 24) & 0xFF000000);
    addr |= (((size_t)bytes[3] << 32) & 0xFF00000000);
    addr |= (((size_t)bytes[2] << 40) & 0xFF0000000000);    
    addr |= (((size_t)bytes[1] << 48) & 0xFF000000000000);
    addr |= (((size_t)bytes[0] << 56) & 0xFF00000000000000); */
    size_t addr = 0;
    memcpy(&addr,bytes,8);
    return addr;
 }
...
//  memcpy(buf + encoded_len, magicNum, 4);
  ...
  index_.insert(input_key, address);
